  ...
  return (
    ...
    >
      ...
      <ModuleBox title={TitleOrderedTests} centered>
        <Row>
          {data.SelectedTests &&
            data.SelectedTests.UncategorizedTests.include &&
            Object.keys(
              data.SelectedTests.UncategorizedTests.include
            ).map(testName =>
              Object.keys(
                data.SelectedTests.UncategorizedTests.include[testName].include
              ).map(display => <Col>{display}</Col>)
            )}
          {data.SelectedDiagnosticTests &&
            data.SelectedDiagnosticTests.map(item => <Col>{item}</Col>)}
        </Row>
      </ModuleBox>
