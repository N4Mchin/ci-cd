...
const ViewSection = props => {
  ...
  const { Hematology } = FHIR_CODES.UncategorizedTests.OtherTests.include
  ...
  const columns = [
    ...
    ...Object.keys(Hematology.include).map(subTestKey => {
      const subTestValue = Hematology.include[subTestKey]

      let relatedReferenceRange

      try {
        relatedReferenceRange = helper.getRelatedReferenceRange(
          Hematology.include[subTestKey].referenceRange,
          patient
        )
      } catch (errorInfo) {
        console.log(errorInfo)
