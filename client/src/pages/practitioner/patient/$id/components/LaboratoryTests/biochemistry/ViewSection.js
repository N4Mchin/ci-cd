...
const ViewSection = props => {
  ...
  return (
    <div className={styles.analysisTable}>
      {Object.keys(BiochemistryTests.include).map(testKey => {
        const displayName = BiochemistryTests.include[testKey].display
        const testColumn = [
          {
            title: <Trans id={'Date'} />,
          ...
          ...Object.keys(BiochemistryTests.include[testKey].include).map(
            subTestKey => {
              const subTestValue =
                BiochemistryTests.include[testKey].include[subTestKey]
