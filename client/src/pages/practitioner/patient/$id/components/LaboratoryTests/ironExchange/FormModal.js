...
const FormModal = props => {
  ...
  const { IronExchange } = props.app.FHIR_CODES.UncategorizedTests
  const IronExchangeInclude = IronExchange.include
      ...
      .then(values => {
        ...
        Object.keys(testResult).forEach(testResultKey => {
          ...
          if (
            ...
            const { referenceRange, unit } = IronExchangeInclude[testResultKey]
      ...
      <ModuleBox title={Title}>
        ...
        >
          <Row gutter={24}>
            <Col span={12}>
              <Row>
                {Object.keys(IronExchangeInclude).map((testKey, index) => {
                  let test

                  try {
                    test = IronExchangeInclude[testKey]
                  } catch {}
