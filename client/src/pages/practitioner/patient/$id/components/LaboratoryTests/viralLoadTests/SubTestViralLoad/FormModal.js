  ...
  const onSubmit = () => {
    ...
    return props.form
      ...
      .then(values => {
        ...
        const testResultValue = testResult['resultValue']
        if (
          !!testResultValue &&
          testResultValue !== '' &&
          !isNaN(parseFloat(testResultValue))
        ) {
          const { referenceRange, unit } = ViralLoadTests.include[
            props.selectedTab
          ]
          valueQuanitities[props.selectedTab] = {
            valueQuantity: {
        ...
        const payload = {
          testKey: props.selectedTab,
          testCode: ViralLoadTests.include[props.selectedTab],
                ...
                <Col span={24}>
                  <Form.Item labelCol={0} wrapperCol={24} help={false}>
                    {getFieldDecorator('resultValue', {
                      ...
                      <EmptyTestItem
                        patient={props.practitioner_patient_profile.patient}
                        test={ViralLoadTests.include[props.selectedTab]}
                        disabled={false}
                      />
                    )}
                  </Form.Item>
