  ...
  const onSubmit = () => {
    ...
    return props.form
      ...
      .then(values => {
        ...
        const testResultValue = testResult['Fibroscan']
        if (
          !!testResultValue &&
          testResultValue !== '' &&
          !isNaN(parseFloat(testResultValue))
        ) {
          const { referenceRange, unit } = Fibroscan
          valueQuanitities['Fibroscan'] = {
            valueQuantity: {
              ...
              <Row>
                <Col span={24}>
                  <Form.Item labelCol={0} wrapperCol={24}>
                    {getFieldDecorator('Fibroscan', {
                      rules: [{ required: false }],
                    })(<EmptyTestItem test={Fibroscan} />)}
                  </Form.Item>
                </Col>
              </Row>
            </Col>
