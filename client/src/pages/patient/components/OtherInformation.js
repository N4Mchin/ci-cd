...
const OtherInformation = props => {
  ...
  useEffect(() => {
    ...
    try {
      const RACE_VALUESET =
        props.laboratory_patientProfile.Valuesets['race-values']
      const OCCUPATION_VALUESET =
        props.laboratory_patientProfile.Valuesets['occupation-values']
...
function onFieldsChange(props, changedFields, allFields) {
  ...
  if (
    ...
    formData.salaryComponent = {
      ...
      valueQuantity: {
        value: parseInt(allFields.salary.value),
        unit: '₮',
      },
    }
  }
  ...
  if (!allFields.occupation.errors && !!allFields.occupation.value) {
    ...
    const system =
      props.laboratory_patientProfile.Valuesets['occupation-values'].compose
        .include[0].system
