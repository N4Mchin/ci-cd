...
import { Button, DatePicker, Input, Col, Row, Tag } from 'antd'
...
const Biochemistry = props => {
  ...
  // '' is empty value of search input
  ...
  const tableProps = {
    ...
    expandedRowRender: record => {
      const subTestNames = []
      Object.keys(record.include).forEach(testKey => {
        Object.keys(record.include[testKey].include).forEach(subTestKey => {
          const subTestName =
            props.app.FHIR_CODES.BiochemistryTests.include[testKey].include[
              subTestKey
            ].display
          subTestNames.push(subTestName)
        })
      })

      return (
        <div style={{ margin: '0', padding: '4px' }}>
          {subTestNames.map(test => (
            <Tag>{test}</Tag>
          ))}
          <Tag style={{ marginTop: '4px' }}>
            <Trans id="Total" />
            :&nbsp;{subTestNames ? subTestNames.length : 0}
          </Tag>
        </div>
      )
    },
  ...
  const addOns = [
    {
      ...
      render: (text, record, index) => {
        return helper.calculateAgeFromBirthDate(record.patient.birthDate)
      },
    },
    {
  ...
  return (
    <Board inner>
      <div style={{ display: 'flex', alignItems: 'flex-end' }}>
        ...
        >
          <span className="bold ">Biochemistry</span>
          {' | '}
          <Trans>
