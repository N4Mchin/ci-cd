...
const ProtocolBlank = props => {
  ...
  const onSaveSpecialProtocolData = () => {
    ...
    form
      ...
      .then(values => {
        let indexedData = {}
        let allValues = {}
        const reduxData = props.laboratory_test_antiHDV
  ...
  //   props
  //     .dispatch({
  //       type: 'laboratory_test/queryExcelDataGenerator',
  //       payload: {
  ...
  //       saveFile(blob, `${props.testName}.xlsx`)
  //     })
  // }
  ...
  const handle = (childData, pageIndex) => {
    ...
    props.dispatch({
      type: 'laboratory_test_antiHDV/updateState',
      payload: {
...
export default connect(
  ({ app, loading, laboratory_test, laboratory_test_antiHDV }) => ({
    app,
    loading,
    laboratory_test,
