...
export default modelExtend(pageModel, {
  namespace: 'laboratory_test_sarsCov2IgGElisa',
  state: {
    modalData: {},
  },
  ...
  effects: {
    *queryLabTests({ payload = {} }, { call, put, select }) {
      ...
      const localState = yield select(
        state => state.laboratory_test_sarsCov2IgGElisa
      )
      const { Sars_Cov2_IgG_Elisa } = localState
      const loincCode = Sars_Cov2_IgG_Elisa.code.coding.find(
    ...
    *queryLabTestResult({ payload = {} }, { call, put, select }) {
      ...
      if (response && response.success) {
        ...
        const dataSource = controller.generateTableData(
          bundle,
          FHIR_CODES,
          FHIR_CODES.UncategorizedTests.OtherTests.include.Sars_Cov2_IgG_Elisa
        )
