...
import ModalHBVTestProtocolPrint from './ModalHBVTestProtocolPrint'
import ModalHBVReagentConsumption from './ModalHBVReagentConsumption'
...
const Title = (
  <div>
    <span className="title">HBV-DNA</span>
    &nbsp;
...
const DailyLogHBV = props => {
  const { dispatch, form, loading, i18n } = props
  const { getFieldDecorator } = form

  const [
    ModalHBVTestProtocolPrintVisible,
    showModalHBVTestProtocolPrint,
  ] = useState(false)

  const [
    ModalHBVReagentConsumptionVisible,
    showModalHBVReagentConsumption,
  ] = useState(false)
  ...
  return (
    <ModuleBox title={Title}>
            ...
            <Row gutter={[10, 10]} style={{ marginTop: '34px' }}>
              ...
              <Col span={24}>
                <Button
                  type="primary"
                  block
                  onClick={() => showModalHBVReagentConsumption(true)}
                >
              ...
              <Col span={24}>
                <Button
                  type="primary"
                  block
                  onClick={() => showModalHBVTestProtocolPrint(true)}
                >
      ...
      <ModalHBVTestProtocolPrint
        visible={ModalHBVTestProtocolPrintVisible}
        onCancel={() => showModalHBVTestProtocolPrint(false)}
        testName="viralLoadTests_HBV"
      />
      <ModalHBVReagentConsumption
        visible={ModalHBVReagentConsumptionVisible}
        onCancel={() => showModalHBVReagentConsumption(false)}
      />
...
)(withI18n()(Form.create()(DailyLogHBV)))
