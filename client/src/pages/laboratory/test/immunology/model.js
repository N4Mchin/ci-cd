...
const { readResource, batch_transaction_request, saveImmunologyProtocol } = api

export default modelExtend(pageModel, {
  namespace: 'laboratory_test_immunologyTests',
  state: {
    Immunology: {},
    localState: {},
    pagination: {},
  ...
  effects: {
    ...
    *queryLabTests({ payload = {} }, { call, put, select }) {
      ...
      const snomedCode = Immunology.code.coding.find(
        code => code.system === FHIR_CODES.SNOMED_URL
      )

      const requestPayload = {
        resourceType: 'ServiceRequest',
        code: `${snomedCode.system}|${snomedCode.code}`,
    ...
    *saveResult({ payload = {} }, { call, put, select }) {
      ...
      const localState = state.laboratory_test_immunologyTests
      ...
      Object.keys(testResult).forEach(testKey => {
        ...
        const observationArray = Object.keys(testResult[testKey].include).map(
          subTestKey => {
            const {
              ...
              referenceRange,
              unit,
            } = globalState.FHIR_CODES.ImmunologyTests.include[testKey].include[
              subTestKey
            ]
        ...
        const parentCode =
          globalState.FHIR_CODES.ImmunologyTests.include[testKey].code
        const parentReferenceRange =
          globalState.FHIR_CODES.ImmunologyTests.include[testKey].referenceRange
      ...
      const diagnosticReportData = {
        ...defaults,
        status: 'partial', //overriding default status
        identifier: [
          Object.assign(
            {
              value: dateTime.getInstant(),
    ...
    *saveProtocolData({ payload }, { call, put, select }) {
      const response = yield call(saveImmunologyProtocol, {
        testName: 'Immunology',
        data: payload,
      })
