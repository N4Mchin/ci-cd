...
const Coagulation = props => {
  const { laboratory_test_coagulation, location, dispatch } = props
  ...
  async function fetchData(tabKey, pagination, filteredDate) {
    let type
    if (tabKey === 'Awaited') {
  ...
  const showModal = (modalName, visible) => {
    dispatch({
      type: 'laboratory_test_coagulation/updateState',
      payload: {
        [modalName]: visible,
      },
    })
  ...
  return (
    <Board inner>
      <div style={{ display: 'flex', alignItems: 'flex-end' }}>
        ...
        >
          <span className="bold ">Coagulation</span>
          {' | '}
          <Trans>
...
export default connect(
  ({ app, laboratory_test, laboratory_test_coagulation, loading }) => ({
    app,
    laboratory_test,
