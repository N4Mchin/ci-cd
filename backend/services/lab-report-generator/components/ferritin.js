...
const TITLE = 'Ferritin ELISA'
const TEST_PROTOCOL = 'БҮ-ЭТ-016'
...
module.exports = (params, writeStream) => {
  ...
  const related = controller.getRelatedReferenceRange(
    LabTests.UncategorizedTests.OtherTests.include.Ferritin.referenceRange,
    patient
  )

  const referenceRangeDisplay = `${related.low.value}-${related.high.value}`

  const testName = params.testCode.display

  tableData.push([testName, `${result}`, unit, referenceRangeDisplay])
